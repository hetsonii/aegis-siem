...
locals {
  # One catch-all route: the dashboard Lambda routes on method + rawPath. This
  # keeps the many console endpoints (findings, stats, incidents, geo,
  # blocklist) in one place and lets us add endpoints without touching Terraform.
  _routes = "single $default route -> dashboard integration"
}
